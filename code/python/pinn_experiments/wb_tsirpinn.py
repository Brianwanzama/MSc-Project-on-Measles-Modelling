# ============================================================
# wb_tsirpinn.py
# TSIR-PINN — West Bengal measles extension
# S comes from TSIR reconstruction (not free parameter)
#
# CHANGES FROM tsirpinn.py (London):
#   1. parquet path: prefit_cases1/
#   2. year_cutoff: float 2017.0
...
#   6. get_data: includes susc_lag_ columns (from parquet)
...
def process(data):
    ...
    WB time: 2008.0, 2008.038... -> biweek 1, 2, 3...
    Births already biweekly in parquet — do NOT divide again.
    """
    data = data.copy()
...
def get_data(data):
    # tsirpinn uses BOTH cases_lag_ AND susc_lag_ features
...
# ── ODE LAYER — TSIR (S from reconstruction, not free param) ──
class derivative_layer(nn.Module):
    ...
    def forward(self, t, SI, Bi, N):
        # S comes from network prediction (trained against TSIR S)
        S    = SI[:, 0:1]
        I    = SI[:, 1:2]
...
def main():
    ...
    print(f"Features: {cases_train.shape[1] - 5} "
          f"(case_lags + susc_lags)")
    ...
    for epoch in range(args.num_epochs):
        ...
        for t, t_ode, S, I, Bi, N, X in train_loader:
            ...
            # tsirpinn: S_pred trained against observed TSIR S
    ...
    stem      = (args.write_loc
                 + f"tsirpinn_k{args.k}_tlag{args.tlag}"
