# ============================================================
# wb_naivepinn_retuned.py
# NaivePINN — West Bengal — RETUNED LOSS WEIGHTS
#
# PROBLEM WITH ORIGINAL:
#   S ~ 300,000, I ~ 15-30 (ratio ~10,000x)
#   λ_I=10, λ_ODE=1 → ODE gradient swamps I gradient
#   S_latent collapses to 0, model ignores physics
#
# FIX:
#   1. Normalise S and I losses by their mean values
#      so both contribute equally regardless of scale
#   2. Increase λ_I to force infectious prediction
#   3. Reduce λ_ODE to prevent trivial zero solution
#   4. Normalise ODE residual by scale of each compartment
# ============================================================
...
class derivative_layer(nn.Module):
    def __init__(self, T, num_t, t_min=1):
        super().__init__()
        self.T        = T
        self.num_t    = num_t
        ...
        self.register_buffer('t_min',
            torch.tensor(t_min, dtype=torch.long))

    def forward(self, t, SI, Bi, N, S_scale, I_scale):
        # Map t to 0-based index
        idx   = (t.long().squeeze() - self.t_min).clamp(
                    0, self.num_t - 1)
        S_lat = torch.exp(
            self.S_latent[idx].unsqueeze(1) + 4.5) * 1e3
        I     = SI[:, 1:2]
        ...
        # ── NORMALISE ODE RESIDUAL BY COMPARTMENT SCALE ──────
        # Prevents S scale (300K) dominating I scale (25)
...
class ode_nn(nn.Module):
    def __init__(self, T, num_t, t_min=1):
        super().__init__()
        self.der = derivative_layer(T, num_t, t_min)
...
def main():
    ...
    # Used to normalise S and I losses to same magnitude
    ...
    print(f"S mean: {S_mean_val:.0f} | I mean: {I_mean_val:.2f}")
    print(f"Scale ratio S/I: {S_mean_val/I_mean_val:.0f}x")
    ...
    all_city = get_data(process(
        pd.concat([get_cities(train_df, [args.city]),
                   get_cities(test_df,  [args.city])])))
    num_t     = int(all_city['time'].max())
    t_min_val = int(t_train.min())

    B     = get_B(scale=0.1).to(device)
    model = NeuralNetwork(
        1, train_data.X.shape[1], 128, 3, B).to(device)
    ode_model = ode_nn(
        T=26, num_t=num_t, t_min=t_min_val).to(device)
    ...
    # ── RETUNED LOSS WEIGHTS ──────────────────────────────────
    # Original: S_hp=0.1, I_hp=10, ode_hp=1
    # Problem:  ODE dominates due to S/I scale mismatch (~10000x)
    # Fix:      Normalise losses + increase I weight + reduce ODE
    S_hp   = 1.0    # weight on S loss (after normalisation)
    I_hp   = 100.0  # weight on I loss (10x increase)
    ode_hp = 0.01   # weight on ODE   (100x decrease)

    print(f"Loss weights: S_hp={S_hp} I_hp={I_hp} | β_max={BETA_MAX} amp_max={AMP_MAX} "
          f"ode_hp={ode_hp}")
    ...
    for epoch in range(args.num_epochs):
        ...
        for t, t_ode, S, I, Bi, N, X in train_loader:
            ...
            t_idx = t.long().squeeze().clamp(0, num_t-1)
            S_latent_batch = list(
                ode_model.parameters())[3][t_idx]

            # ── NORMALISED LOSSES ─────────────────────────────
            # Divide by scale so S and I losses are comparable
            loss_S = loss_fn(S_pred / S_scale,
                             S_latent_batch.unsqueeze(1)
                             / S_scale)
            loss_I = loss_fn(I_pred / I_scale,
                             I / I_scale)

            # ODE loss uses normalised residuals
    ...
    stem = (args.write_loc
            + f"naivepinn_constrained_v3_k{args.k}"
            + f"_tlag{args.tlag}"
