...
runtsir_updated <- function (data, xreg = "cumcases", IP = 2, nsim = 10, regtype = "gaussian", 
                             ...
                             mul.noise.sd = 0, printon = F, fit = NULL, fittype = NULL) 
{
  ...
  return(list(X = X, Y = Y, Yhat = Yhat, contact = contact, 
              ...
              time = data$time, inits.grid = inits.grid, inits = IC, S = S, I = I))
}

k_step_ahead <- function(t, t_lag, k, dat) {
  fit_data <- dat[(t - t_lag + 1):t, ]
  
  fit_data_forecasts <- dat[(t + 1):(t + k), c("time")]
  fit_data_forecasts$k <- 1:k
  
  #tsir doesn't work when all cases are 0. predict 0 for all steps ahead in this case
  if (sum(fit_data$cases == 0) >= nrow(fit_data) - 1) {
    
    fit_data_forecasts$cases <- 0
    
  } else {
    
    #runtsir suggests using break method when percent zero cases > 0.3
    ...
    fit <- runtsir_updated(data=fit_data, IP=2,
                           xreg='cumcases',regtype='lm',
                           alpha=0.98, sbar=NULL,
                           method='deterministic',
                           nsim=1, #this doesn't matter for deterministic
                           pred = 'step-ahead',
                           family='poisson',
                           link='log',
                           sigmamax = 0.5,
                           epidemics = epidemics)
    S_fit <- c(fit$S, rep(0, k))
    I_fit <- c(fit$I, rep(0, k))
    period_fit <- c(fit$period, (fit$period[length(fit$period)] + 1), rep(0, k))
    if (period_fit[length(fit$period) + 1] == 27) {
      period_fit[length(fit$period) + 1] <- 1
    }
    beta_fit <- unname(fit$beta)
    alpha_fit <- 0.98
    rho_fit <- fit$rho[length(fit$rho)]
    
    for (i in 1:k) {
      period_fit[t_lag + i] <- period_fit[t_lag + i - 1] + 1
      if (period_fit[t_lag + i] == 27) period_fit[t_lag + i] <- 1
      
      I_fit[t_lag + i] <- unname(beta_fit[period_fit[t_lag + i]]) * S_fit[t_lag + i - 1] * (I_fit[t_lag + i - 1] ^ alpha_fit)
      
      S_fit[t_lag + i] <- fit_data$births[(t_lag + period_fit[t_lag + i]) - 26] + S_fit[t_lag + i - 1] - I_fit[t_lag + i]
      
      #one step ahead (use births/pop from one year prior
    }
    
    fit_data_forecasts$cases <- I_fit[(t_lag + 1):(t_lag + k)] / rho_fit
  }
  
  
  return(fit_data_forecasts)
}


get_preds_one_city <- function(dat, train_index, k, t_lag = 130) {
  indices_to_fit <- train_index:(nrow(dat))
  all_dat <- vector(mode = 'list', length = length(indices_to_fit)) # 
  
  for (i in 1:length(indices_to_fit)) {
    all_dat[[i]] <- k_step_ahead(t = indices_to_fit[i], t_lag = t_lag, k = k, dat = dat)
    print(paste0("t = ", indices_to_fit[i], ", stop at t = ", indices_to_fit[length(indices_to_fit)]))
  }
  all_dat_df <- Reduce(rbind, all_dat)
  return(all_dat_df)
}
